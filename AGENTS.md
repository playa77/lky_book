...
- **Chapter level:** with a global doc in force, a chapter either carries no style/tone instruction (inherits the global doc verbatim — the uniform-work case) or carries a modulation that must remain **within the global doc's tonal family**. Without a global doc, every chapter must carry an instruction (§7.4). In nf works with leaf = chapter, chapter and unit level coincide.
...
Commit the proposal, present the report, and stop. The user's approval must confirm the mode and, for nf, the leaf granularity; record both in `STATE.md`. Do not expand anything until the audit is confirmed accurate.
...
- Scope `scoped`: proceed; chapter instructions will be proposed per §7.4 where missing.

Record the scope in `STATE.md` in the approval commit. No expansion begins before this gate is passed.

### 7.3 Iterative expansion — **GATE per unit of expansion**
...
- The user may accept, edit, reject, or request re-expansion. On edits to unit or word counts, recalculate the chapter total. Commit the approval, then proceed to the next chapter.

### 7.4 Chapter-level style/tone (scoped and global+modulation)

- In `scoped` scope, each chapter carries a style/tone instruction — the baseline register for its units. If the outline lacks one, propose one from the chapter's content and the work's genre, subject, and register, and get confirmation before expanding that chapter.
...
Bottom-up only: unit pacing → unit target; chapter = sum of units; part = sum of chapters (where parts exist); work = sum of the top level. The user may override any number; totals recalculate. If the user sets a chapter target and asks for redistribution, propose a unit distribution that respects pacing logic.

### 7.6 Normalized outline — **GATE**

When every chapter is expanded and approved, assemble `outline/normalized-outline.md`:

- Header states the mode, leaf granularity (nf), style scope, and — if global docs are in force — names them.
- Every unit: global ordinal `S<n>` (assigned in strict reading order across the whole work), plus its full attribute set per §7.3 step 4, and target word count.
...
Exit criteria — verify all five before presenting: (1) every leaf is a drafting unit; (2) every unit has an effective style/tone instruction per the inheritance chain; (3) every chapter total reconciles with its units; (4) no register discontinuity anywhere in the Work → Chapter → Unit chain; (5) explicit user approval of the document. On approval: commit, tag `phase1-complete`, create the `manuscript/` directory skeleton. The normalized outline is now part of the `O` tier and binding.
